...
function check()
{
	...
	do
		...
		if [[ $flags == *"--save"* ]]; then
			mv bitmap.bmp ./script/outputs/$(echo "$file" | tr '/' '_' | sed 's/\._/.\//g')_$(python3 -c 'from time import time; print(int(round(time() * 1000)), end="");').bmp
		fi
	done
}
...
function check_bmp_files()
{
	...
	count=$(ls -l ./script/outputs/*.bmp | wc -l | tr -d " ")
	if [ $count -ge 1 ]
	then
		g_bmp_files_present=1
		printf "\033[90G%s\033[94G\033[0;32m %s\033[0m\n" "✅"
		open ./script/outputs/*.bmp
	else
		printf "\033[90G%s\033[94G\033[0;31m %s\033[0m\n" "❌"
	fi
}
...
check "./script/maps/scene_valid_*" 0 "" 15 true

category "Testing valid map (with --save flag)"
check "./script/maps/scene_valid_*" 0 "--save" 80 true

category "Testing valid map (with --save flag AND garbage)"
check "./script/maps/scene_valid_*" 0 "--save hello i am the garbage" 80 true

category "Checking BMP files"
check_bmp_files

category "Testing invalid map"
check "/i/dont/exists ./script/maps/scene_broken*.cub" 1 "" 15 false
